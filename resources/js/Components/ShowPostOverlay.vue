...
const textareaInput = (e) => {
    ...
    textarea.value.height = `${e.target.scrollHeight}px`;
};
</script>
...
<template>
            ...
            <div class="w-full md:flex h-full overflow-auto rounded-xl">
                ...
                <div class="md:max-w-[500px] w-full relative">
                    ...
                    >
                        ...
                        ></textarea>
                        <button v-if="comment" @click="$event => { $emit('addComment', { post, user, comment }) ; comment = '' }" class="text-blue-600 font-extrabold pr-4">
                            Post
                        </button>
                    </div>
                </div>
    ...
    <ShowPostOptionsOverlay v-if="deleteType" :deleteType="deleteType" :id="id" @deleteSelected="$event => { $emit('deleteSelected', {
        ...
        deleteType = null;
        id = null;
    }" @clode="$event => { deleteType = null; id = null }" />
     />
</template>
